 ...
 * with possible modifications from Mark Nadal
 */
var classes = [];
var lastClass = 0;
...
function escapeUnicode() {
	...
	var fn = function(parser, pos, cla) {
		parser.unicode = parser.unicode * 16 + parseInt(String.fromCharCode(parser.data[pos]), 16);
		parser.keep.push(Buffer.from(String.fromCharCode(parser.unicode), 'utf8'));
		parser.beg = pos + 1;
		return INSIDE_QUOTES;
	};
...
function Parser(callback, callbackDepth, done) {
...
Parser.prototype.update = function(str) {
	if (typeof str === "string") str = Buffer.from(str, 'utf8');
	this.data = str;
